...
def update_weights(config, network, optimizer, batch, train_results):
    ...
    def loss():
        ...
        state_representation, V, policy_logits = network.initial_model.call(np.array(state_batch))
        ...
        l_v =  tf.math.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(target_value_batch, V))
        l_p = tf.cast(tf.math.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits = policy_logits, labels = target_policy_batch)), tf.float32)
        ...
        for actions_batch, targets_batch in zip(actions_batch, targets_recurrent_batch):
            ...
            combined_representation = tf.concat((state_representation, actions_batch), axis=1)
            state_representation, reward_batch, value_batch, policy_batch = network.recurrent_model(combined_representation)
            ...
            l_v =  tf.math.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(target_value_batch, value_batch))
            l_p = tf.cast(tf.math.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits = policy_batch, labels = target_policy_batch)), tf.float32)
            
            #reward loss
            l_r = tf.math.reduce_mean(MSE(target_reward_batch, reward_batch))

            
            # Remember to scale value loss!
            # Add to total losses
            total_value_loss +=.25* l_v
            total_policy_loss+= l_p
            total_reward_loss += l_r
            
            # YOUR CODE HERE: Half the gradient of the representation
            state_representation = scale_gradient(state_representation, 0.5)
            
            #print('Scaled State: ', state_representation)

            # YOUR CODE HERE: Sum the losses, scale gradient of the loss, add to overall loss
            
            l_step = .25* l_v + l_p + l_r

            #l_step /= config.num_unroll_steps            
            l_step = scale_gradient(l_step, 1/config.num_unroll_steps)
